"""Alias mappings, expected types, and semantic patterns for standard procurement fields."""
...
def _norm(s: str) -> str:
    """Lowercase, strip, collapse whitespace/underscores/hyphens."""
...
_RAW_ALIASES: dict[str, list[str]] = {
    "Invoice Number": [
        ...
        "invoice_id", "inv_id", "rechnungsnummer", "facture",
    ],
    "Invoice Line Number": [
        "line_no", "line_num", "inv_line", "invoice_line", "line_number", "line item",
        "item_no", "inv_line_no", "invoice line", "lineitem",
    ],
    "Invoice Date": [
    ...
    "Total Amount paid in Reporting Currency": [
        ...
        "total amount reporting", "amount reporting currency",
    ],
    "Price per UOM": [
    ...
    "PO Document Date": [
        "po_date", "po_doc_date", "purchase_order_date", "po date",
    ],
    ...
    "PO Material Group Code": [
        ...
        "material group code", "category_code", "material_group_code",
    ],
    ...
    "Vendor Code": [
        ...
        "vendor code", "supplier code", "lifnr",
    ],
    "Vendor Name": [
        "vendor_name", "supplier_name", "vendor", "supplier",
        "vendor name", "supplier name", "vendor_nm", "supplier_nm",
    ],
    "Vendor Country": [
    ...
    "Cost Center Description": [
        ...
        "cost center description", "cost center name",
    ],
    "GL Account": [
        "gl_account", "gl_code", "gl", "general_ledger", "saknr",
        "gl account", "gl code", "account_code",
    ],
...
# ---------------------------------------------------------------------------
# 2. Expected data types per field
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Semantic pattern detectors  (applied to sample values)
# ---------------------------------------------------------------------------
...
SEMANTIC_TAG_TO_FIELDS: dict[str, list[str]] = {
    ...
    "country_iso2": [
        "Vendor Country", "Company Country", "Plant Country",
    ],
    "country_name": [
        "Vendor Country", "Company Country", "Plant Country",
    ],
...
for tag, fields in SEMANTIC_TAG_TO_FIELDS.items():
    for field in fields:
        FIELD_TO_SEMANTIC_TAGS.setdefault(field, []).append(tag)
