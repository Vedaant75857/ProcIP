"""Country and region normalisation via AI + SQL updates."""
...
country_normalisation_bp = Blueprint("country_normalisation", __name__)

_DISTINCT_LIMIT = 5000


def _load_country_prompts():
    path = (
    ...
    spec = importlib.util.spec_from_file_location("m2_country_prompts", path)
...
@country_normalisation_bp.route("/fix-supplier-country", methods=["POST"])
def fix_supplier_country():
    p = _load_country_prompts()
    try:
        ...
        col = _find_column(
            cols,
            ["country", "nation", "ctry", "supplier country", "vendor country"],
            body.get("column"),
        )
        if not col:
            return jsonify({"error": "Could not resolve country column."}), 400

        unique = column_distinct_values(conn, sql_name, col, limit=_DISTINCT_LIMIT)
        api_key = app_state.get("openai_api_key") or None
        clear_progress()
        update_progress(0, 1, "Country normalisation: starting AI batches…")

        def _cb(done, total, eta):
            update_progress(done, total, "Country normalisation: AI batches", eta_seconds=eta)

        mapping, tracker = batch_ai_mapping(
            unique,
            p.SYSTEM_PROMPT_FIX_COUNTRY,
            p.INSTRUCTIONS_FIX_COUNTRY,
            api_key=api_key,
            progress_cb=_cb,
...
@country_normalisation_bp.route("/fix-regions", methods=["POST"])
def fix_regions():
    p = _load_country_prompts()
    try:
        body = request.get_json(silent=True) or {}
        ...
        col = _find_column(
            cols,
            ["region", "geo", "territory", "area", "continent"],
            body.get("column"),
        )
        if not col:
            return jsonify({"error": "Could not resolve region column."}), 400

        unique = column_distinct_values(conn, sql_name, col, limit=_DISTINCT_LIMIT)
        api_key = app_state.get("openai_api_key") or None
        clear_progress()
        update_progress(0, 1, "Region normalisation: starting AI batches…")

        def _cb(done, total, eta):
            update_progress(done, total, "Region normalisation: AI batches", eta_seconds=eta)

        mapping, tracker = batch_ai_mapping(
            unique,
            p.SYSTEM_PROMPT_FIX_REGIONS,
            p.INSTRUCTIONS_FIX_REGIONS,
            api_key=api_key,
            progress_cb=_cb,
            cost_tracker=CostTracker(),
        )
