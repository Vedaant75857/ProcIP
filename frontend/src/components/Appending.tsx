...
import { Loader2, ChevronRight, ChevronDown, LayoutGrid, ListChecks, ArrowRight, Info, Plus, MoreHorizontal, Undo2, Ban, X, Eye, EyeOff, MessageSquare, Sparkles, AlertTriangle, Lightbulb, RefreshCw, Database, Columns, BarChart3, TrendingUp, Shield, Layers } from "lucide-react";
...
/* ─── Insight Content (shared between panel and inline) ─── */
  ...
  return (
    <div className="space-y-4">
      ...
      {insight.dataDescription && (
        <span className="inline-block text-[10px] font-bold bg-red-50 dark:bg-red-950/30 text-red-500 dark:text-red-400 px-2 py-0.5 rounded-full">
          {insight.dataDescription}
        </span>
      )}
...
function InsightsPanel({
  ...
  const currentInsight = selectedGroupId ? groupInsights[selectedGroupId] : null;
  const currentReport = groupReports?.find((r: any) => r.groupId === selectedGroupId);
  const hasAnyInsight = Object.keys(groupInsights).length > 0;
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 z-50 h-full w-full max-w-lg bg-white dark:bg-neutral-900 border-l border-neutral-200 dark:border-neutral-700 shadow-2xl flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 shrink-0">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-red-500" />
                <h3 className="font-bold text-neutral-900 dark:text-white text-sm">Group Insights</h3>
              </div>
              <div className="flex items-center gap-2">
                {onRetryInsights && (
                  <button
                    ...
                    disabled={groupInsightsLoading}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-600 dark:text-neutral-300 hover:border-red-300 hover:text-red-600 transition-all disabled:opacity-50"
                  >
                    {groupInsightsLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />}
                    {hasAnyInsight ? "Refresh" : "Generate"}
                  </button>
                )}
                <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-500 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            {/* Group tabs */}
            {appendGroups.length > 1 && (
              <div className="flex gap-1 px-4 py-2 border-b border-neutral-100 dark:border-neutral-800 overflow-x-auto shrink-0">
                {appendGroups.map((g) => {
                  const hasIns = !!groupInsights[g.group_id];
                  return (
                    <button
                      key={g.group_id}
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold whitespace-nowrap transition-all ${
                        selectedGroupId === g.group_id
                          ...
                          : "bg-neutral-50 dark:bg-neutral-800/50 text-neutral-400 hover:bg-neutral-100"
                      }`}
                    >
                      {g.group_name || g.group_id}
                      {hasIns && <span className="ml-1 w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" />}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {groupInsightsLoading && !currentInsight ? (
                ...
                <InsightContent insight={currentInsight} report={currentReport} />
              ) : (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <Sparkles className="w-8 h-8 text-neutral-300 dark:text-neutral-600 mb-3" />
                  <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-4">
                    {selectedGroupId ? "No insight available for this group yet." : "Select a group tab above to view insights."}
                  </p>
                  {onRetryInsights && (
                    <button
                      type="button"
                      onClick={onRetryInsights}
                      className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold text-neutral-500 dark:text-neutral-400 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl hover:border-red-300 hover:text-red-600 transition-all"
                    >
...
export default function Appending({
  ...
  const [selectedInsightGroup, setSelectedInsightGroup] = useState<string | null>(null);
  ...
  const openInsightsForGroup = (groupId: string) => {
    setSelectedInsightGroup(groupId);
    setInsightsPanelOpen(true);
  };

  const handleGenerateAndOpen = () => {
    if (onRetryInsights) onRetryInsights();
    if (appendGroups.length > 0) {
      setSelectedInsightGroup(appendGroups[0].group_id);
    }
    setInsightsPanelOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <InsightsPanel
        ...
        selectedGroupId={selectedInsightGroup}
        onSelectGroup={setSelectedInsightGroup}
      />
          ...
          <div className="p-6 border-b border-neutral-100 dark:border-neutral-800">
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-2 shrink-0">
                {appendGroups.length > 0 && (
                  <button
                    type="button"
                    onClick={handleGenerateAndOpen}
                    disabled={groupInsightsLoading}
                    className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg shadow-sm text-white bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 transition-all disabled:opacity-50"
                  >
                    {groupInsightsLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                    Generate Insights
                  </button>
                )}
                      ...
                      <motion.div key={group.group_id} whileHover={{ y: -1 }} className="border border-neutral-200 dark:border-neutral-700 rounded-2xl bg-white dark:bg-neutral-900 hover:border-red-200 dark:hover:border-red-900/50 transition-all shadow-sm overflow-visible">
                        ...
                        <div className="flex justify-between items-start p-5 pb-0">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => openInsightsForGroup(group.group_id)}
                        ...
                        {/* Body: files, preview, insight summary */}
                        <div className="p-5">
                          <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-4 leading-relaxed line-clamp-2">{group.reason}</p>

                          {/* Inline insight summary (compact) */}
                          {hasInsight && (
                            <div
                              onClick={() => openInsightsForGroup(group.group_id)}
                              className="mb-4 p-3 rounded-xl border border-red-100 dark:border-red-900/30 bg-red-50/50 dark:bg-red-950/20 cursor-pointer hover:border-red-200 transition-colors"
                            >
                              <div className="flex items-center gap-2 mb-1">
                                <Sparkles className="w-3 h-3 text-red-500" />
                                <span className="text-[10px] font-bold text-red-600 dark:text-red-400 uppercase tracking-wider">AI Insight</span>
                                <span className="text-[10px] text-neutral-400 ml-auto">Click to expand</span>
                              </div>
                              {insight.summary && (
                                <p className="text-xs text-neutral-600 dark:text-neutral-300 line-clamp-2">{insight.summary}</p>
                              )}
                            </div>
                          )}
